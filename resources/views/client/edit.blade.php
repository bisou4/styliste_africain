...
@section('content')
    <div class="container">
        <h2 class="text-center mt-3 text-primary shadow p-3 mb-5 bg-body rounded">Formulaire de modification</h2>
        <h5 class="text-center mb-3">Veuillez insérer les données à modifier dans le formulaire</h5>
        <form method="POST" class="shadow p-3 mb-5 bg-body rounded"
            action="{{ route('gestion_client.update', [$finds->id]) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="d-flex">
                <div>
                ...
                <div>
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" name="telephone" value="{{ $finds->telephone }}"
                            id="exampleInputEmail1" aria-describedby="emailHelp">
                    </div>
                    <div class="mb-3">
